...
buffer.forEach((bufferStep,index,arr) => {
    ...
    const startBuffer = endStepTime[bufferStep.after.unit][bufferStep.after.step]
    const endBuffer = Math.round((startBuffer + bufferStep.time))


    //////////////////////
    function genBufferToResultTable(unit: string, step: StepData, start: number, end: number) {
        if (result[unit]) {
            result[unit] = [
                ...result[unit],
                {
